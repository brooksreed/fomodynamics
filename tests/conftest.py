...
# required for physics reasons (see docs/dev/testing.md for exceptions).
...
class _NoopArtifactSaver:
    """Module-level fallback stub used when no validation tree is available.

    The public tree only ships the no-op stub; private downstream consumers
    can drop in their own ArtifactSaver via the conftest hook.
    """
