...
class LQRController(ControlSchedule):
    ...
    @classmethod
    def for_quadrotor_scg_compatible(
        cls,
        quadrotor: "JaxDynamicSystem",
        Q_scale: float = 1.0,
        R_scale: float = 0.1,
    ) -> tuple["LQRController", dict]:
        """Create LQR controller for quadrotor with SCG-compatible tuning.

        Safe-control-gym (SCG) uses a 12-state Euler angle representation,
        while BLUR uses a 13-state quaternion representation. This factory
        handles the 13->12 state reduction needed for LQR design.

        The 13-state system has controllability rank 12 because qw is
        constrained by the unit quaternion requirement (qw^2 + qx^2 + qy^2 + qz^2 = 1).
        At hover (qw=1, qx=qy=qz=0), small attitude changes only affect qx, qy, qz.

        State reduction:
            - 13-state: [pos(3), vel(3), qw, qx, qy, qz, omega(3)]
            - 12-state: [pos(3), vel(3), qx, qy, qz, omega(3)]  (qw removed)

        The method:
            1. Linearizes at hover equilibrium
            2. Reduces to 12-state system by removing qw (index 6)
            3. Solves continuous-time LQR with SCG-like weights
            4. Embeds the 12-state gain back into 13-state format

        Quaternion handling:
            The 13-state system uses quaternions [qw, qx, qy, qz] for attitude.
            At hover, qw ≈ 1 and qx, qy, qz ≈ 0. The linearized system has
            controllability rank 12 because qw is constrained by the unit
            quaternion requirement: qw = sqrt(1 - qx² - qy² - qz²).

            For small perturbations from hover, only qx, qy, qz vary freely,
            so we design the LQR controller on the reduced 12-state system
            (removing qw) and then embed the gains back into 13-state format
            with zero gain on the qw column.

        Args:
            quadrotor: QuadrotorJax instance (or compatible 13-state system)
            Q_scale: Multiplier for state cost weights (default 1.0)
            R_scale: Multiplier for control cost weights (default 0.1)

        Returns:
            Tuple of:
                - LQRController with 13-state gain matrix
                - Analysis dict with keys:
                    - K: Full 13-state gain (4, 13)
                    - K_reduced: 12-state gain used for design (4, 12)
                    - Q: State cost matrix (12, 12)
                    - R: Control cost matrix (4, 4)
                    - eigenvalues: Closed-loop eigenvalues (12,)
                    - max_eigenvalue_mag: Max |real(lambda)| for stability check
                    - dt_limit: Recommended max timestep for RK4 stability

        Example:
            from fmd.simulator import Quadrotor
            from fmd.simulator.lqr import LQRController

            quad = Quadrotor()
            controller, info = LQRController.for_quadrotor_scg_compatible(quad)

            print(f"Max eigenvalue: {info['max_eigenvalue_mag']:.1f}")
            print(f"Recommended dt < {info['dt_limit']*1000:.2f} ms")
        """
        # 1. Get hover equilibrium
        x_eq = quadrotor.default_state()  # 13-state hover
        u_eq = quadrotor.hover_control()  # [mg, 0, 0, 0]

        # 2. Linearize the 13-state system
        A13, B13 = linearize(quadrotor, x_eq, u_eq)

        # 3. Reduce to 12-state system by removing qw (index 6)
        # State indices to keep: [0,1,2,3,4,5, 7,8,9, 10,11,12]
        # This removes qw (index 6) which is constrained at hover
        keep_indices = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12]
        A12 = A13[np.ix_(keep_indices, keep_indices)]
        B12 = B13[keep_indices, :]

        # 4. Build Q and R matrices with SCG-like weights
        # 12-state order: [pos_n, pos_e, pos_d, vel_u, vel_v, vel_w,
        #                  qx, qy, qz, omega_p, omega_q, omega_r]
        q_diag = np.array([
            Q_scale,  # pos_n
            Q_scale,  # pos_e
            Q_scale,  # pos_d
            ...
            Q_scale,  # vel_v
            Q_scale,  # vel_w
            Q_scale,  # qx (roll-like)
            Q_scale,  # qy (pitch-like)
            Q_scale,  # qz (yaw-like)
            Q_scale,  # omega_p
            Q_scale,  # omega_q
            Q_scale,  # omega_r
        ...
        Q12 = np.diag(q_diag)
        R = np.diag([R_scale, R_scale, R_scale, R_scale])

        # 5. Solve continuous-time LQR on 12-state system
        K12 = compute_lqr_gain(A12, B12, Q12, R, discrete=False)

        # 6. Embed K12 in 13-state gain matrix
        # K13 has shape (4, 13), with zero column for qw
        K13 = np.zeros((4, 13))
        K13[:, :6] = K12[:, :6]      # pos, vel gains
        K13[:, 6] = 0.0              # qw has zero gain
        K13[:, 7:10] = K12[:, 6:9]   # qx, qy, qz gains
        K13[:, 10:13] = K12[:, 9:12]  # omega gains

        # 7. Compute closed-loop eigenvalues for analysis
        A12_np = np.asarray(A12)
        B12_np = np.asarray(B12)
        K12_np = np.asarray(K12)
        A_cl = A12_np - B12_np @ K12_np
        eigenvalues = linalg.eigvals(A_cl)

        # Max magnitude of real parts (for stability analysis)
        max_eigenvalue_mag = np.max(np.abs(eigenvalues.real))

        # RK4 stability limit: dt < 2.785 / |lambda_max|
        dt_limit = 2.785 / max_eigenvalue_mag if max_eigenvalue_mag > 0 else np.inf

        # 8. Build analysis dict
        analysis = {
            "K": jnp.array(K13),
            "K_reduced": jnp.array(K12),
            "Q": jnp.array(Q12),
            "R": jnp.array(R),
            "eigenvalues": eigenvalues,
            "max_eigenvalue_mag": float(max_eigenvalue_mag),
            "dt_limit": float(dt_limit),
        }

        # Return controller and analysis
        controller = cls(
            K=jnp.array(K13),
            x_ref=jnp.asarray(x_eq),
            u_ref=jnp.asarray(u_eq),
        )
