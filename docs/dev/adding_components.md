...
See `TestCartpoleEquivalence` or `TestBox1DEquivalence` in `tests/simulator/casadi/test_equivalence.py` for reference implementations.
...
This package (`fomodynamics`, import `fmd`) is a self-contained public
layer. Code in `fmd.*` may not depend on private downstream packages.

| If you're adding... | Put it in... |
|---------------------|--------------|
| Generic sim infrastructure (base classes, integrator, control) | `fmd.simulator` |
| Generic analysis (DataStream, processing, plots) | `fmd.analysis` |
| Core math (quaternion, circular ops, units) | `fmd.core` |
| Moth forces, params, model changes | Existing moth files in `fmd.simulator` |
| Generic OCP infrastructure (MultipleShootingOCP), estimation | `fmd.ocp`, `fmd.estimation` |
| New toy / teaching models | `fmd.simulator` |

If you're unsure where something belongs, ask. When in doubt, keep it
small and focused — it's easier to promote a helper into the public
surface later than to retract it.
